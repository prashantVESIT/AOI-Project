...
const WAQI_API_KEY = "YOUR_API_KEY";
...
export async function getCoordinates(city) {
    ...
    const response = await fetch(
        ...
        `${GEO_URL}?q=${encodeURIComponent(city)}&limit=10&appid=${OPENWEATHER_API_KEY}`
...
export async function getCityData(city){
    ...
    pm25:
    aqiData.data?.iaqi?.pm25?.v || 0,


    pm10:
    aqiData.data?.iaqi?.pm10?.v || 0,


    co:
    aqiData.data?.iaqi?.co?.v || 0,


    no2:
    aqiData.data?.iaqi?.no2?.v || 0,


    so2:
    aqiData.data?.iaqi?.so2?.v || 0,


    o3:
    aqiData.data?.iaqi?.o3?.v || 0
